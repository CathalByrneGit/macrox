...
**Optional Python backends** (Docling for ML table detection, GLiNER2 for local NLP):
...
| Mode | How |
|---|---|
| **Standalone app** | `mx_app()` — full GUI, no code needed |
| **Script API** | Build interactively, save as YAML macro, replay automatically |
| **Shiny module** | `macrox_ui()` / `macrox_server()` — embed in any existing app |
