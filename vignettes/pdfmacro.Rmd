---
title: "pdfmacro: PDF Table Extraction Workflows"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{pdfmacro: PDF Table Extraction Workflows}
...
pdfmacro records every step of a PDF table extraction workflow as a YAML macro and replays it automatically on new files.  Think of it as Power Query for PDFs — interactive exploration that compiles into a reproducible pipeline.
...
install.packages(c(
  ...
  "reticulate",  # PaddleOCR backend
  "xml2",        # PaddleOCR HTML parsing
...
# Open a session
sess <- pdf_session("report_2024.pdf")
...
# Replay next year
pdf_replay("report_2025.pdf", macro = "my_report")
```
...
```{r app}
pdf_app()           # system browser
pdf_app("dialog")   # RStudio dialog
pdf_app("pane")     # Viewer pane
```
...
| Tab | Purpose |
|---|---|
| **Extract** | Full PDF viewer, draw-to-select area, method picker, live preview. `←` / `→` navigate pages. |
| **Tables** | One tab per extracted table. Rename, cast types, filter rows (GUI), validate rules, column stats. |
| **Items** | Extract single metadata fields (invoice numbers, dates, totals) via LLM. |
...
pdfmacro ships four extraction engines.  They can be mixed freely within the same session and macro.
...
**Supported providers:** `provider` can be the name of any
[ellmer](https://ellmer.tidyverse.org/) chat constructor, without the
`chat_` prefix — e.g. `"anthropic"`, `"openai"`, `"google_gemini"`,
`"openrouter"`, `"groq"`, `"mistral"`, `"deepseek"`, `"ollama"`, or
`"openai_compatible"`. `model` defaults to a sensible choice for
`"anthropic"`, `"openai"`, and `"google_gemini"`; other providers require
`model` to be set explicitly.

For OpenAI-compatible endpoints (LM Studio, vLLM, etc.) or hosted gateways
like OpenRouter:
...
### paddle — PP-StructureV3 (PaddleOCR, offline)

Renders the page image and runs it through Baidu's PP-StructureV3 table recognition pipeline.  Works completely offline — no API keys, no network calls after initial model download.  Best for scanned PDFs and complex layouts where `bbox` struggles.

```{r paddle-setup, eval = FALSE}
# One-time setup (restart R after)
setup_paddle()                        # CPU — installs onnxruntime backend (~20 MB)
setup_paddle(cuda_version = "12.6")   # GPU — CUDA 12.6

# Restricted network? Download wheels on an internet machine first:
download_paddle_wheels("~/paddleocr_wheels", python_version = "3.9")
# copy folder to server, then:
setup_paddle(wheels_dir = "/path/to/paddleocr_wheels")
```

```{r paddle}
area <- sess |> locate_area(page = 40)

sess |> select_table_paddle("stillborn", page = 40, area = area)

# Choose backend explicitly when both are installed
sess |> select_table_paddle("tbl", page = 5, backend = "onnxruntime")
sess |> select_table_paddle("tbl", page = 5, backend = "paddle", device = "gpu")
```
...
tables <- pdf_replay("report_2025.pdf", macro = "my_report")

# Batch replay across multiple files
files   <- list.files("reports/", pattern = "\\.pdf$", full.names = TRUE)
results <- pdf_replay_batch(files, macro = "my_report")
```
...
Embed pdfmacro in an existing Shiny app:
...
| Scanned PDF, no selectable text | `paddle` or `llm` |
| Offline / air-gapped / no API key | `bbox` or `paddle` |
...
| `rlang` | `%\|\|%` operator | Yes |
| `magick` | Page annotation; image cropping for LLM/paddle | Suggests |
| `ellmer` | LLM extraction | Suggests |
| `reticulate` | PaddleOCR via Python | Suggests |
| `xml2` | Parse PP-StructureV3 HTML output | Suggests |
